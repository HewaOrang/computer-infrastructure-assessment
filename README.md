...
This project downloads, processes, plots, and automates data for the five FAANG stocks — **META, AAPL, AMZN, NFLX, GOOG**.  
...
- Uses `yfinance` to download **hourly data from the last 5 days** for the above five FAANG stocks.
- Saves the output into a **data** folder.
- Names each file using a date and time stamp format: `YYYYMMDD-HHmmss.csv`.

This ensures the repository consistently stores clean, date and time-stamped datasets.

---

## **Problem 2 – Plotting Data**

This step processes and plots the most recent dataset:
...
This produces easy to read visual summary for each dataset collected.
...
- The `faang.py` explains how the script is constructed and configured.
...
- Runs the script **every Saturday morning @ 10:30am**.
- Installs dependencies and then **executes** `./faang.py`.
- The  `faang.yaml` provides a line-by-line explanation of the workflow.
